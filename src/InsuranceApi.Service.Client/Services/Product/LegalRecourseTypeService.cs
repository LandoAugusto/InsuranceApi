...
namespace InsuranceApi.Service.Client.Services.Product
{
    ...
    internal class LegalRecourseTypeService(ILogWriter logWriter, IHttpClientFactory httpClientFactory, IOptions<ApiConfig> option) : ILegalRecourseTypeService
    {
        ...
        public async Task<IEnumerable<LegalRecourseTypeModel>?> GetAllAsync()
        {
            ...
            try
            {
                ...
                rawRequest.RequestUri = $"{_endpont.Url}/v1/LegalRecourseType/get-all";
                rawResponse = await _httpClient.GetAsync<RawRequest, RawResponse>(rawRequest.RequestUri, rawRequest);
                var response = JsonConvert.DeserializeObject<BaseDataResponseModel<IEnumerable<LegalRecourseTypeModel>>>(rawResponse.Conteudo);
