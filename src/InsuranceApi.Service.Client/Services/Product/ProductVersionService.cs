        ...
        : IProductVersionService
    {
        ...
        public async Task<IEnumerable<ConstructionTypeModel?>> GetConstructionTypeAsync(int productVersionId, int profileId)
        {
            ...
            try
            {
                ...
                rawRequest.RequestUri = $"{_endpont.Url}/v1/ProductVersion/get-product-version-construction-type/{productVersionId}/{profileId}";
                rawResponse = await _httpClient.GetAsync<RawRequest, RawResponse>(rawRequest.RequestUri, rawRequest);
        ...
        public async Task<IEnumerable<AssistanceModel>?> GetPlanAssistanceAsync(int productVersionId, int planId)
        {
            ...
            try
            {
                ...
                rawRequest.RequestUri = $"{_endpont.Url}/v1/ProductVersion/get-product-version-plan-assistance/{productVersionId}/{planId}";
                rawResponse = await _httpClient.GetAsync<RawRequest, RawResponse>(rawRequest.RequestUri, rawRequest);
                var response = JsonConvert.DeserializeObject<BaseDataResponseModel<IEnumerable<AssistanceModel?>>>(rawResponse.Conteudo);
                if (!response.TransactionStatus.Sucess)
                {
                    if (response.TransactionStatus.Code == 404)
                    {
        ...
        public async Task<IEnumerable<QuestionnaireModel>> GetQuestionnaireAsync(int productVersionId)
        {
            ...
            try
            {
                ...
                rawRequest.RequestUri = $"{_endpont.Url}/v1/ProductVersion/get-product-version-questionnaire/{productVersionId}";
                rawResponse = await _httpClient.GetAsync<RawRequest, RawResponse>(rawRequest.RequestUri, rawRequest);
                var response = JsonConvert.DeserializeObject<BaseDataResponseModel<IEnumerable<QuestionnaireModel>>>(rawResponse.Conteudo);
                if (!response.TransactionStatus.Sucess)
                {
                    if (response.TransactionStatus.Code == 404)
                    {
